...
const labels = {
  vi: { subtitle: 'Theo dõi doanh thu và giao dịch trong ca hiện tại.', updated: 'Cập nhật gần nhất', ticketType: 'Vé trực tiếp', cash: 'Tiền mặt', qr: 'Chuyển khoản QR', completed: 'Hoàn tất', pending: 'Chờ thanh toán', empty: 'Chưa có dữ liệu trong ca này.' },
  en: { subtitle: 'Track revenue and transactions for the current shift.', updated: 'Last updated', ticketType: 'Walk-in ticket', cash: 'Cash', qr: 'QR transfer', completed: 'Completed', pending: 'Pending', empty: 'No data for this shift.' },
};
...
const ShiftRevenuePage = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const response = await busAssistantService.getShiftRevenue();
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        <Section title={t.revenueBreakdown} icon={ReceiptText}>
          <div className="overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="bg-slate-50 text-xs uppercase tracking-wider text-slate-500"><tr><th className="px-5 py-3">{t.type}</th><th className="px-5 py-3 text-center">{t.tickets}</th><th className="px-5 py-3 text-right">{t.revenue}</th></tr></thead><tbody className="divide-y divide-slate-100">{data?.revenueBreakdown?.length ? data.revenueBreakdown.map((item) => <tr key={item.ticketType} className="hover:bg-emerald-50/40"><td className="px-5 py-4 font-semibold text-slate-800">{item.ticketType === 'WALK_IN' ? l.ticketType : item.ticketType}</td><td className="px-5 py-4 text-center">{item.tickets}</td><td className="px-5 py-4 text-right font-bold text-emerald-900">{money(item.revenue)}</td></tr>) : <tr><td colSpan="3" className="px-5 py-10 text-center text-slate-500">{l.empty}</td></tr>}</tbody></table></div>
        </Section>
