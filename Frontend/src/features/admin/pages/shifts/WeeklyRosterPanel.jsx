...
import { AlertTriangle, CalendarDays, CheckCircle2, RefreshCw, Send, Trash2, Wand2, X } from 'lucide-react';
...
export default function WeeklyRosterPanel({ weekStartDate }) {
  ...
  const cancellableShifts = useMemo(() => (data?.rows || []).filter((row) => dateKey(row.workDate) >= tomorrowKey()), [data]);
  const tripAllocatedShifts = useMemo(() => cancellableShifts.filter((row) => row.tripAssignments?.length), [cancellableShifts]);
  ...
  const removeDriverShift = async () => {
    const shift = editing?.shiftId;
    if (!shift || roleFilter !== 'DRIVER') return;
    if (dateKey(editing.workDate) < dateKey(new Date())) return toast.error('Không thể gỡ ca trong quá khứ. Bạn có thể gỡ ca hôm nay hoặc trong tương lai.');
    if (!window.confirm(`Gỡ ca ${shift.startTime}–${shift.endTime} ngày ${dateKey(editing.workDate)} khỏi tài xế này? Ca làm, phụ xe và xe vẫn được giữ lại và lịch tuần sẽ chuyển sang trạng thái thiếu tài xế.`)) return;
    setBusy(true);
    try {
      await adminService.removeDriverFromSelectedShift(id(shift));
      toast.success('Đã gỡ ca của tài xế. Lịch tuần cần được phân công lại trước khi công bố.');
      setEditing(null);
      await load();
    } catch (error) { toast.error(error?.message || 'Không thể gỡ ca của tài xế.'); }
    finally { setBusy(false); }
  };

  const removeAssistantShift = async () => {
    const shift = editing?.shiftId;
    if (!shift || roleFilter !== 'ASSISTANT') return;
    if (dateKey(editing.workDate) < dateKey(new Date())) return toast.error('Không thể gỡ ca trong quá khứ. Bạn có thể gỡ ca hôm nay hoặc trong tương lai.');
    if (!window.confirm(`Gỡ ca ${shift.startTime}–${shift.endTime} ngày ${dateKey(editing.workDate)} khỏi phụ xe này? Ca làm, tài xế và xe vẫn được giữ lại.`)) return;
    setBusy(true);
    try {
      await adminService.removeAssistantFromSelectedShift(id(shift));
      toast.success('Đã gỡ ca của phụ xe. Lịch tuần cần được phân công lại.');
      setEditing(null);
      await load();
    } catch (error) { toast.error(error?.message || 'Không thể gỡ ca của phụ xe.'); }
    finally { setBusy(false); }
  };
  ...
  const cancelAllShifts = async () => {
    const activeShiftCount = cancellableShifts.length;
    if (!activeShiftCount) return toast('Tuần này không có ca từ ngày mai trở đi để hủy. Ca trong hôm nay và các ngày trước không thể hủy.');
    if (!window.confirm(`HỦY ${activeShiftCount} CA TỪ NGÀY MAI TRỞ ĐI trong tuần ${dateKey(data.days[0])} – ${dateKey(data.days[6])}?\n\nCa trong hôm nay và các ngày trước sẽ được giữ nguyên. Phân công nhân sự, xe và liên kết chuyến của các ca đủ điều kiện sẽ bị hủy. Thao tác này không thể hoàn tác.`)) return;
    setBusy(true);
    try {
      const result = await adminService.cancelAllWeeklyRosterShifts(weekStartDate);
      setValidation(null);
      setEditing(null);
      toast.success(`Đã hủy ${result.cancelledShifts || 0} ca trong lịch tuần.`);
      if (result.protectedCurrentOrPastShifts) toast(`${result.protectedCurrentOrPastShifts} ca trong hôm nay hoặc trước đó được giữ nguyên.`);
      if (result.cancelledTripAllocations) toast(`Đã gỡ ${result.cancelledTripAllocations} liên kết phân bổ chuyến.`);
      await load();
    } catch (error) { toast.error(error?.message || 'Không thể hủy toàn bộ ca trong tuần.'); }
    finally { setBusy(false); }
  };

  const isPublished = data?.roster?.status === 'PUBLISHED';
  const hasShifts = Boolean(data?.rows?.length);
  const publishHint = isPublished
    ? 'Lịch đã công bố và đang được khóa.'
    : !hasShifts
      ? 'Hãy cấu hình nhu cầu và sinh lịch trước.'
      : validation?.valid === true
        ? 'Lịch hợp lệ và sẵn sàng công bố.'
        : 'Hãy kiểm tra lịch và xử lý toàn bộ lỗi bắt buộc trước khi công bố.';
  ...
  return <div className="space-y-5">
    <section className="rounded-3xl border border-emerald-100 bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex items-center gap-2"><CalendarDays className="text-emerald-700" /><h2 className="text-xl font-black">Lịch phân ca tuần · {dateKey(data.days[0])} – {dateKey(data.days[6])}</h2></div><p className="mt-1 text-sm text-slate-500">Mỗi hàng là một nhân viên, mỗi cột là một ngày. Ô “Nghỉ” nghĩa là nhân viên không có ca trong ngày đó.</p><div className="mt-3 flex flex-wrap gap-2 text-xs font-bold"><span className="rounded-full bg-sky-100 px-3 py-1 text-sky-800">Ca sáng · 05:30–12:00</span><span className="rounded-full bg-orange-100 px-3 py-1 text-orange-800">Ca chiều · 12:00–18:30</span><span className="rounded-full bg-slate-100 px-3 py-1 text-slate-600">Nghỉ</span></div></div><span className={`rounded-full px-4 py-2 text-xs font-black ${isPublished ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'}`}>{isPublished ? 'Đã công bố' : 'Bản nháp'}</span></div>
      {!hasShifts ? <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50/70 p-3">
        <p className="mb-3 text-xs font-black uppercase tracking-[0.16em] text-slate-500">Quy trình lập lịch tuần</p>
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between"><div className="grid flex-1 gap-2 sm:grid-cols-2 lg:grid-cols-4"><button disabled={busy || isPublished} onClick={openRequirements} className="h-11 rounded-xl border border-emerald-300 bg-white px-3 text-sm font-black text-emerald-800 disabled:opacity-40"><span className="mr-2 text-emerald-500">1</span>Cấu hình nhu cầu</button><button disabled={busy || isPublished} onClick={autoGenerate} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-emerald-600 px-3 text-sm font-black text-white disabled:opacity-40"><span className="text-emerald-200">2</span><Wand2 size={16} /> Sinh lịch tự động</button><button disabled={busy || !hasShifts} onClick={() => run(() => adminService.validateWeeklyRoster(weekStartDate), 'Đã kiểm tra lịch tuần.')} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white px-3 text-sm font-black disabled:opacity-40"><span className="text-slate-400">3</span><CheckCircle2 size={16} /> Kiểm tra lịch</button><button disabled={busy || validation?.valid !== true || isPublished} onClick={publishRoster} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-[#062819] px-3 text-sm font-black text-white disabled:opacity-40"><span className="text-emerald-300">4</span><Send size={16} /> Công bố lịch</button></div><button disabled={busy} onClick={load} title="Tải lại dữ liệu" className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-slate-300 bg-white"><RefreshCw className={busy ? 'animate-spin' : ''} size={17} /></button></div>
        <div className={`mt-3 rounded-xl px-3 py-2 text-xs font-bold ${validation?.valid === true || isPublished ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'}`}>{publishHint}</div>
      </div> : null}
      <div className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-rose-100 bg-rose-50/50 px-4 py-3"><div><p className="text-sm font-black text-slate-800">Thao tác quản trị</p><p className="text-xs text-slate-500">Các thao tác này có thể thay đổi toàn bộ lịch tuần.</p></div>{isPublished ? <button disabled={busy} onClick={() => run(() => adminService.reopenWeeklyRoster(weekStartDate), 'Đã mở lịch để chỉnh sửa có kiểm soát.')} className="h-10 rounded-xl border border-amber-300 bg-white px-4 text-sm font-black text-amber-800">Mở lịch để chỉnh sửa</button> : <button disabled={busy || !hasShifts} onClick={cancelAllShifts} className="inline-flex h-10 items-center gap-2 rounded-xl border border-rose-300 bg-white px-4 text-sm font-black text-rose-700 disabled:opacity-40"><Trash2 size={17} /> Hủy toàn bộ ca trong tuần</button>}</div>
    </section>

    {tripAllocatedShifts.length ? <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900"><b>{tripAllocatedShifts.length} ca đang được chuyến sử dụng.</b><p className="mt-1 text-xs">Nếu hủy toàn bộ ca tuần, hệ thống cũng sẽ gỡ các liên kết phân bổ chuyến này rồi hủy ca.</p></div> : null}
    ...
    {shortages.length ? <section className="rounded-2xl border border-rose-200 bg-rose-50 p-4"><div className="flex items-center gap-2 font-black text-rose-800"><AlertTriangle size={18} /> Thiếu nguồn lực sau khi sinh lịch tự động</div><div className="mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-3">{shortages.map((item, index) => <div key={`${item.date}-${item.routeId}-${item.shift}-${item.type}-${index}`} className="rounded-xl bg-white p-3 text-sm"><b>{item.routeCode || item.routeId} · {item.date} · {item.shift === 'MORNING' ? 'Ca sáng' : 'Ca chiều'}</b><p className="mt-1 text-rose-700">{item.type === 'DRIVER' ? 'Tài xế' : item.type === 'ASSISTANT' ? 'Phụ xe' : 'Xe'}: {item.assigned}/{item.required} ⚠ · thiếu {item.shortage}</p></div>)}</div></section> : null}
