...
import { AlertTriangle, CheckCircle2, ListChecks, Pencil, RefreshCw, Route, UsersRound } from 'lucide-react';
...
export default function TripAllocationPanel({ onEditSchedule, onSaved, routes }) {
  ...
  return <div className="space-y-5">
    ...
    {summary ? <section className="overflow-hidden rounded-3xl border border-emerald-100 bg-white shadow-sm">
      ...
      <div className="max-h-[420px] overflow-auto"><table className="w-full min-w-[1040px] text-left text-sm"><thead className="sticky top-0 bg-slate-50 text-xs font-black uppercase text-slate-500"><tr>{['Mã chuyến', 'Chiều', 'Thời gian', 'Vòng D–V', 'Tài xế', 'Phụ xe', 'Xe', 'Trạng thái', 'Thao tác'].map((label) => <th key={label} className="px-4 py-3">{label}</th>)}</tr></thead><tbody>{visibleTrips.map((trip) => <tr key={trip._id} className="border-t border-slate-100 hover:bg-emerald-50/30"><td className="px-4 py-3 font-black">{trip.scheduleCode}</td><td className="px-4 py-3">{trip.direction === 'OUTBOUND' ? 'Chiều đi' : 'Chiều về'}</td><td className="whitespace-nowrap px-4 py-3 font-bold">{trip.departureTime}–{trip.expectedArrivalTime}</td><td className="px-4 py-3 text-slate-500">{trip.operationCycleCode || '—'}</td><td className="px-4 py-3">{trip.driverName || <span className="font-bold text-rose-600">Chưa có</span>}</td><td className="px-4 py-3">{trip.assistantName || <span className="font-bold text-rose-600">Chưa có</span>}</td><td className="px-4 py-3">{trip.vehicleLabel || <span className="font-bold text-rose-600">Chưa có</span>}</td><td className="px-4 py-3"><span className={`rounded-full px-3 py-1 text-xs font-black ${trip.assigned ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'}`}>{trip.assigned ? 'Đã giao' : `Thiếu ${trip.missing.join(', ') || 'phân công'}`}</span></td><td className="px-4 py-3"><button type="button" onClick={() => onEditSchedule?.(trip)} className="inline-flex h-9 items-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50 px-3 text-xs font-black text-emerald-800 hover:bg-emerald-100"><Pencil size={14} /> Sửa</button></td></tr>)}</tbody></table>{!visibleTrips.length ? <div className="p-8 text-center text-sm font-bold text-slate-500">{tripFilter === 'UNASSIGNED' ? 'Không còn chuyến chưa giao.' : 'Không có chuyến trong bộ lọc đã chọn.'}</div> : null}</div>
    </section> : null}
