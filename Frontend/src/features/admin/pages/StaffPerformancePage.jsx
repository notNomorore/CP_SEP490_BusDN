...
const StaffPerformancePage = () => {
  ...
  const filteredStaffMembers = useMemo(() => {
    ...
    return staffPerformance.staffMembers.filter((member) => {
      ...
      const haystack = [member.fullName, member.email, member.phone, member.phoneNumber, member.role]
        .filter(Boolean)
        .join(' ')
        .toLowerCase();
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className={`truncate text-[11px] ${textSecondaryClassName}`}>{member.email || member.phone || member.phoneNumber || 'Chưa có thông tin liên hệ'}</p>
                          </div>
                        </div>
