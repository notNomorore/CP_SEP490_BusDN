...
const PASSENGER_LOST_ITEM_STATUSES = ['REPORTED', 'SEARCHING', 'FOUND', 'RETURNED', 'UNRECOVERED'];
...
export const CreateSupportCaseDTO = {
  validate: (body) => {
    ...
    if (body.ratingScore && (Number(body.ratingScore) < 1 || Number(body.ratingScore) > 5)) {
...
export const UpdatePassengerLostItemCaseDTO = {
  validate: (body) => {
    const errors = {};
    if (body.status && !CASE_STATUSES.includes(body.status)) errors.status = 'Status is invalid';
    if (body.recoveryStatus && !PASSENGER_LOST_ITEM_STATUSES.includes(body.recoveryStatus)) {
